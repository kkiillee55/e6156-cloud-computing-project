...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password,form.password.data):
            ...
            flash('Wrong password dor username\n','danger')
...
def new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash('Post:{} created'.format(form.title.data),'success')
...
def send_reset_email(user):
    ...
    msg.body='To reset password, user link below:\n' \
             '{}\n' \
             'Ignore if you didnt request it\n'.format(url_for('reset_token',token=token,_external=True))
    mail.send(msg)
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('PW reset email sent to you','success')
        return redirect(url_for('login'))
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('PW updated for {}'.format(user.username),'success')
        return redirect(url_for('login'))
